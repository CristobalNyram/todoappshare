        ...
        <div class="breadcrumb-with-buttons mb-24 flex-between flex-wrap gap-8">
            ...
            <div class="flex-align gap-8 flex-wrap">
                    ...
                    <div class="flex-align text-gray-500 text-13 border border-gray-100 rounded-4 ps-20 focus-border-main-600 bg-white">
                        ...
                        <input type="text" id="filtro-usuario" class="form-control ps-8 pe-20 py-16 border-0 text-inherit rounded-4" placeholder="Filtrar por usuario">
                    </div>
            ...
            <div class="container mt-5 bg-white shadow-sm rounded-4 p-4">
                ...
                <div class="table-responsive mb-5 shadow-sm rounded-4 overflow-hidden">
                    <table id="tabla-usuarios" class="table table-hover align-middle table-bordered mb-0 bg-white h6 mb-0 fw-medium text-gray-300">
                        <thead class="table-light">
                            <tr>
                                <th class="h6 text-gray-300">ID</th>
                                <th class="h6 text-gray-300">Usuario</th>
                    ...
                    <div class="modal fade" id="modalNuevoUsuario" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-centered">
                                <form id="form-nuevo-usuario" class="modal-content shadow rounded-4">
                    ...
                    <div class="modal fade" id="modalEditarUsuario" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <form id="form-editar-usuario" class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" name="paterno" placeholder="Apellido" required>
                                            </div>
                                            <div class="col-md-12">
                            ...
                            <div class="modal-dialog modal-md">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-center">
                                        <p class="fs-5 mb-3">¿Estás seguro de que deseas cambiar el estatus del usuario?</p>
                                        <input type="hidden" id="cambio-estatus-id-usuario">
...
<script>
const dtUserTable = $('#tabla-usuarios');
...
$(function () {
    ...
    const url = BASE_URL_API + 'V1/Users/?action=list';
    ...
    if (dtUserTable.length) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        // Botón de editar
                        const btnEditar = `
                            <button class="btn btn-icon btn-primary btn-editar" data-id="${row.id_usuario}" data-bs-toggle="tooltip" title="Editar Usuario">
                                <i class="fas fa-edit"></i>
                            </button>`;
                        // Botón de cambiar estatus
                        const btnEstatus = esActivo 
                            ? `<button type="button" class="btn btn-icon btn-danger btn-cambiar-estatus" data-id="${row.id_usuario}" data-estatus="2" title="Desactivar Usuario">
                                    <i class="fas fa-user-slash"></i>
                            </button>`
                            : `<button type="button" class="btn btn-icon btn-success btn-cambiar-estatus" data-id="${row.id_usuario}" data-estatus="1" title="Activar Usuario">
        ...
        $('#form-nuevo-usuario').on('submit', function (e) {
            ...
            const token = localStorage.getItem('Tk');
            const url = BASE_URL_API + 'V1/Users/?action=create';

            const formData = {
                nombre: $('[name="nombre"]').val(),
                usuario: $('[name="usuario"]').val(),
                contrasenia: $('[name="contrasenia"]').val(),
                paterno: $('[name="paterno"]').val(),
            };
        ...
        // Rellenar campos del usuario a editar
        $('#tabla-usuarios tbody').on('click', '.btn-editar', function () {
            const tabla = $('#tabla-usuarios').DataTable(); // Aquí era 'tabla-usuarios', no 'tabla-alumnos'
            const data = tabla.row($(this).closest('tr')).data();

            $('#form-editar-usuario [name="id_usuario"]').val(data.id_usuario);
            $('#form-editar-usuario [name="usuario"]').val(data.usuario);
            $('#form-editar-usuario [name="nombre"]').val(data.nombre);
            $('#form-editar-usuario [name="paterno"]').val(data.paterno);
            $('#form-editar-usuario [name="contrasenia"]').val(''); // Opcional: vacío para no mostrar contraseña
        ...
        $('#form-editar-usuario').on('submit', function (e) {
            ...
            const token = localStorage.getItem('Tk');
            const url = BASE_URL_API + 'V1/Users/?action=edit';

            const formData = {
                id_usuario: $('#form-editar-usuario [name="id_usuario"]').val(),
                nombre: $('#form-editar-usuario [name="nombre"]').val(),
                usuario: $('#form-editar-usuario [name="usuario"]').val(),
                paterno: $('#form-editar-usuario [name="paterno"]').val(), 
                contrasenia: $('#form-editar-usuario [name="contrasenia"]').val()
            };
            ...
            $.ajax({
                ...
                success: function () {
                    ...
                    $('#tabla-usuarios').DataTable().ajax.reload();  // Aquí también estaba mal como 'tabla-alumnos'
        ...
        $('#tabla-usuarios tbody').on('click', '.btn-cambiar-estatus', function () {
            ...
            $('#cambio-estatus-id-usuario').val(id_usuario);
            $('#cambio-estatus-valor').val(nuevoEstatus);
            
            // Actualizar texto del modal según el tipo de cambio
            $('#modalCambiarEstatusLabel').text(`Confirmar ${estatusTexto} usuario`);
            $('.modal-body p').text(`¿Estás seguro de que deseas ${estatusTexto} este usuario?`);
        ...
        $('#btn-confirmar-cambio').on('click', function() {
            const token = localStorage.getItem('Tk');
            const url = BASE_URL_API + 'V1/Users/?action=edit-estatus';
            
            const id_usuario = $('#cambio-estatus-id-usuario').val();
