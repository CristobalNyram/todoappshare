    ...
    <div class="breadcrumb-with-buttons mb-24 flex-between flex-wrap gap-8">
            <div class="breadcrumb mb-24">
                <ul class="flex-align gap-4">
                    ...
                    <li><span class="text-main-600 fw-normal text-15">Teachers</span></li>
                </ul>
            </div>
        ...
        <div class="container mt-5 bg-white shadow-sm rounded-4 p-4">
                ...
                <div class="table-responsive mb-5 shadow-sm rounded-4 overflow-hidden">
                    <table id="tabla-profesores" class="table table-hover align-middle table-bordered mb-0 bg-white h6 mb-0 fw-medium text-gray-300">
                        <thead class="table-light">
                            <tr>
                                <th class="h6 text-gray-300">Profesor ID</th>
                                <th class="h6 text-gray-300">Usuario ID</th>
                                <th class="h6 text-gray-300">Nombre</th>
                                <th class="h6 text-gray-300">Apellido Paterno</th>
                                <th class="h6 text-gray-300">Apellido Materno</th>
                                <th class="h6 text-gray-300">Especialidad</th>
                                <th class="h6 text-gray-300">Biografia</th>
                                <th class="h6 text-gray-300">Avatar</th>
                        ...
                        <!--  Modal para agregar nuevo profesor  -->
                    <div class="modal fade" id="modalNuevoUsuario" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <form id="form-nuevo-profesor" class="modal-content shadow rounded-4">
                                        <div class="modal-header bg-light border-bottom-0">
                                            <h5 class="modal-title" id="modalLabel"> Nuevo Profesor</h5>
                                        ...
                                        <div class="modal-body row g-3 px-4 py-3">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-floating">
                                                    <input type="text" name="contrasenia" class="form-control" placeholder="Constraseña">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-floating">
                                                    <textarea name="biografia" class="form-control" placeholder="Biografía" style="height: 100px;"></textarea>
                    ...
                    <div class="modal fade" id="modalEditarUsuario" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <form id="form-editar-profesor" class="modal-content" enctype="multipart/form-data">
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body text-center">
                                            <p class="fs-5 mb-3">¿Estás seguro de que deseas cambiar el estatus del profesor?</p>
                                            <input type="hidden" id="cambio-estatus-id-profesor">
...
<script>
    const dtUserTable = $('#tabla-profesores');
    
    $(function () {
        'use strict';

        const token = localStorage.getItem('Tk');
        const url = BASE_URL_API + 'V1/Teachers/?action=list';
        ...
        if (dtUserTable.length) {
            const tabla = dtUserTable.DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        d.nombre = $('#filtro-nombre').val().trim();
                        delete d.columns;
                        delete d.order;
                        delete d.search;
                        delete d.start;
                ...
                columns: [
                    ...
                    { data: 'biografia'},
                    {
                        ...
                        render: function(data) {
                            return `<img src="${data}" alt="Avatar" class="rounded-circle" width="50" height="50">`;
                        }
                    },
                    { 
                        data: 'estatus',
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            // Botón de editar
                            const btnEditar = `
                                <button class="btn btn-icon btn-primary btn-editar" data-id="${row.id_profesor}" data-bs-toggle="tooltip" title="Editar Usuario">
                                    <i class="fas fa-edit"></i>
                                </button>`;
                            // Botón de cambiar estatus
                            const btnEstatus = esActivo 
                                ? `<button type="button" class="btn btn-icon btn-danger btn-cambiar-estatus" data-id="${row.id_profesor}" data-estatus="2" title="Desactivar Usuario">
                                        <i class="fas fa-user-slash"></i>
                                </button>`
                                : `<button type="button" class="btn btn-icon btn-success btn-cambiar-estatus" data-id="${row.id_profesor}" data-estatus="1" title="Activar Usuario">
            ...
            // Enviar formulario para crear un nuevo profesor
            $('#form-nuevo-profesor').on('submit', function (e) {
                e.preventDefault();

                const token = localStorage.getItem('Tk');
                const url = BASE_URL_API + 'V1/Teachers/?action=create';
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#form-nuevo-profesor')[0].reset();
                        $('#tabla-profesor').DataTable().ajax.reload();
                    ...
                    error: function (xhr) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error al crear usuario: ' + (xhr.responseJSON?.message || xhr.statusText)
                        });
                    }
                });
            });

            // Rellenar campos del profesor a editar
            $('#tabla-profesores tbody').on('click', '.btn-editar', function () {
                const tabla = $('#tabla-profesores').DataTable();
                const data = tabla.row($(this).closest('tr')).data();

                $('#form-editar-profesor [name="id_usuario"]').val(data.id_usuario);
                $('#form-editar-profesor [name="id_profesor"]').val(data.id_profesor);
                $('#form-editar-profesor [name="nombre"]').val(data.nombre);
                $('#form-editar-profesor [name="usuario"]').val(data.usuario);
                $('#form-editar-profesor [name="apellido_p"]').val(data.apellido_p);
                $('#form-editar-profesor [name="apellido_m"]').val(data.apellido_m);
                $('#form-editar-profesor [name="especialidad"]').val(data.especialidad);
                $('#form-editar-profesor [name="biografia"]').val(data.biografia);
                $('#form-editar-profesor [name="avatar"]').val('');
                $('#form-editar-profesor [name="contrasenia"]').val('');
            ...
            $('#form-editar-profesor').on('submit', function (e) {
                ...
                const token = localStorage.getItem('Tk');
                const url = BASE_URL_API + 'V1/Teachers/?action=edit';

                const formData = {
                    id_usuario: $('#form-editar-profesor [name="id_usuario"]').val(),
                    id_profesor: $('#form-editar-profesor [name="id_profesor"]').val(),
                    nombre: $('#form-editar-profesor [name="nombre"]').val(),
                    usuario: $('#form-editar-profesor [name="usuario"]').val(),
                    apellido_p: $('#form-editar-profesor [name="apellido_p"]').val(),
                    apellido_m: $('#form-editar-profesor [name="apellido_m"]').val(),
                    especialidad: $('#form-editar-profesor [name="especialidad"]').val(),
                    biografia: $('#form-editar-profesor [name="biografia"]').val(),
                    avatar: $('#form-editar-profesor [name="avatar"]').val(),
                    contrasenia: $('#form-editar-profesor [name="contrasenia"]').val()
                };
                ...
                $.ajax({
                    ...
                    success: function () {
                        ...
                        $('#tabla-profesores').DataTable().ajax.reload();
            ...
            $('#tabla-profesores tbody').on('click', '.btn-cambiar-estatus', function () {
                const id_profesor = $(this).data('id');
                const nuevoEstatus = $(this).data('estatus');
                const estatusTexto = nuevoEstatus === '1' || nuevoEstatus === 1 ? 'activar' : 'desactivar';
                
                $('#cambio-estatus-id-profesor').val(id_profesor);
                $('#cambio-estatus-valor').val(nuevoEstatus);
                
                // Actualizar texto del modal según el tipo de cambio
                $('#modalCambiarEstatusLabel').text(`Confirmar ${estatusTexto} profesor`);
                $('.modal-body p').text(`¿Estás seguro de que deseas ${estatusTexto} este profesor?`);
            ...
            $('#btn-confirmar-cambio').on('click', function() {
                const token = localStorage.getItem('Tk');
                const url = BASE_URL_API + 'V1/Teachers/?action=edit-estatus';
                
                const id_profesor = $('#cambio-estatus-id-profesor').val();
                    ...
                    success: function() {
                        ...
                        const estatusTexto = nuevoEstatus === '1' || nuevoEstatus === 1 ? 'activado' : 'desactivado';
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: '¡Éxito!',
                                text: `Profesor ${estatusTexto} correctamente`
                            });
