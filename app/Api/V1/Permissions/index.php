...
require_once __ROOT_DIR__ . 'Api/V1/Permissions/services.php';
require_once __ROOT_DIR__ . 'Api/V1/Permissions/validators.php';
...
try {
    switch ($action) {
        case 'list':
            ...
            $errors = validateListPermissions($request, $meedo);
            if (!empty($errors)) {
            ...
            $responseComplete = permissionsIndex($request);
            break;

            case 'create':
                (new HttpMethodGuard())->allowOnly(['POST']);
                $errors = validateCreatePermissions($request, $meedo);
                if (!empty($errors)) {
                    $statusCodeResponse = 400;
                    $statusResponse = false;
                    $messageResponse = "Datos inválidos...";
                ...
                $statusCodeResponse = 200;
                $responseComplete = permissionsCreate($request);
                break;
